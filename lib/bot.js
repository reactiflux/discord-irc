...
class Bot {
  ...
  parseText(message) {
    ...
    return text
      ...
      .replace(/<#(\d+)>/g, (match, channelId) => {
        const channel = this.discord.getChannel('id', channelId);
        return '#' + channel.name;
      });
  }
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      const colorIndex = (username.charCodeAt(0) + username.length) % this.nickColors.length;
      const coloredUsername = irc.colors.wrap(this.nickColors[colorIndex], username);
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (discordChannelName) {
      ...
      const discordChannel = this.discord.getChannel('name', discordChannelName.slice(1));
