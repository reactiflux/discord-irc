...
class Bot {
  ...
  parseText(message, server) {
    const text = message.mentions.reduce((content, mention) => {
      const displayName = getDiscordNicknameOnServer(mention, server);
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      const nickname = getDiscordNicknameOnServer(author, fromServer);
      let text = this.parseText(message, fromServer);
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (discordChannelName) {
      ...
      const withMentions = text.replace(/@[^\s]+\b/g, match => {
        // TODO DarkSpyro003: Mentions with nicknames? What about usernames & nicknames with spaces?
