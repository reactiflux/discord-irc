...
class Bot {
  ...
  static getDiscordNicknameOnServer(user, server) {
    const userDetails = server.detailsOfUser(user);
    return userDetails.nick || user.username;
  }

  parseText(message) {
    const text = message.mentions.users.reduce((content, mention) => {
      const displayName = Bot.getDiscordNicknameOnServer(mention, message.server);
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      const nickname = Bot.getDiscordNicknameOnServer(author, fromServer);
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (discordChannelName) {
      ...
      const withMentions = text.replace(/@[^\s]+\b/g, (match) => {
        let user = this.discord.users.find('username', match.substring(1));
        const server = discordChannel.server;
        if (user) {
          if (Bot.getDiscordNicknameOnServer(user, server).toUpperCase()
          !== match.substring(1).toUpperCase()) {
            user = null; // Nickname doesn't match mention
          }
        }
        if (!user) {
          const length = this.discord.users.length;
          for (let i = 0; i < length && !user; i += 1) {
            if (Bot.getDiscordNicknameOnServer(this.discord.users[i], server).toUpperCase()
            === match.substring(1).toUpperCase()) {
              user = this.discord.users[i];
            }
          }
        }
        return user || match;
