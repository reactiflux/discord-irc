...
class Bot {
  constructor(options) {
    ...
    _.forOwn(options.webhooks, (url, chan) => {
      let urlParts = url.split('/').slice(-2);
      this.webhooks[chan] = {
        'id': urlParts[0],
        'token': urlParts[1]
      }
  ...
  sendToIRC(message) {
    ...
    if (author.id === this.discord.user.id ||
        Object.keys(this.webhooks).some((channel) => {return this.webhooks[channel].id === author.id})
      ) return;
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (discordChannelName) {
      ...
      if (this.webhooks[discordChannelName]) {
        this.webhooks[discordChannelName].client.sendSlackMessage({
          'username': author,
          'text': text
        }).catch(console.error);
        return;
      ...
      const withMentions = text.replace(/@[^\s]+\b/g, (match) => {
        ...
        if (user) {
          const nickname = guild.members.find('id', user.id).nickname;
