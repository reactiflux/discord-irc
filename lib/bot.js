...
class Bot {
  constructor(options) {
    ...
    // IRC clients/Discord nicks to ignore
  ...
  ignoredIrcUser(nick, to, message) {
    return (typeof message !== 'undefined')
      && ('prefix' in message)
      && (this.ignoreUsers.irc.some(mask =>
        new RegExp(`^[^!]+!${mask}$`, 'i').test(message.prefix) === true));
  }

  ignoredDiscordUser(message) {
    return (typeof message !== 'undefined')
      && ('author' in message)
      && (this.ignoreUsers.discord.some(i =>
        i.toLowerCase() === message.author.username.toLowerCase()));
  }
  ...
  attachListeners() {
    ...
    this.discord.on('message', (message) => {
      ...
      if (!this.ignoredDiscordUser(message)) {
    ...
    this.ircClient.on('message', (nick, to, text, message) => {
      ...
      if (!this.ignoredIrcUser(nick, to, message)) {
        this.sendToDiscord(nick, to, text);
      }
    });

    this.ircClient.on('notice', (nick, to, text, message) => {
      // Do not send to Discord if this user is on the ignore list.
      if (!this.ignoredIrcUser(nick, to, message)) {
        this.sendToDiscord(nick, to, `*${text}*`);
      }
