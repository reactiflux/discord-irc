...
class Bot {
  constructor(options) {
    ...
    this.ircNickColor = options.ircNickColor || true;
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.ircNickColor) {
        displayUsername = irc.colors.wrap(NICK_COLORS[(username.charCodeAt(0) + username.length) % NICK_COLORS.length], username);
      }
