...
class Bot {
  ...
  static getDiscordNicknameOnServer(user, guild) {
    const userDetails = guild.members.find('id', user.id);
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (discordChannelName) {
      ...
      const withMentions = text.replace(/@[^\s]+\b/g, (match) => {
        const search = match.substring(1).toUpperCase();
        const server = discordChannel.guild;
        let user = server.members.find('username', match.substring(1));
        if (user) {
          const userNickname = Bot.getDiscordNicknameOnServer(user, server).toUpperCase();
          if (userNickname !== search) {
            user = null; // Nickname doesn't match mention
          }
        }
        if (!user) {
          user = server.members.find('nickname', match.substring(1));
        }
        return user || match;
      });
