...
class Bot {
  ...
  getDiscordNicknameOnServer(user, server) {
  ...
  parseText(message) {
    const text = message.mentions.reduce((content, mention) => {
      const displayName = this.getDiscordNicknameOnServer(mention, message.server);
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      const nickname = this.getDiscordNicknameOnServer(author, fromServer);
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (discordChannelName) {
      ...
      const withMentions = text.replace(/@[^\s]+\b/g, match => {
        let user = this.discord.users.get('username', match.substring(1));
        const server = discordChannel.server;
        if (user) {
          if (this.getDiscordNicknameOnServer(user, server).toUpperCase()
        ...
        if (!user) {
          ...
          for (let i = 0; i < length && !user; i++) {
            if (this.getDiscordNicknameOnServer(this.discord.users[i], server).toUpperCase()
            === match.substring(1).toUpperCase()) {
