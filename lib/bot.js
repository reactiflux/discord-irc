...
class Bot {
  constructor(options) {
    ...
    this.ircIgnoreUsers = this.ircIgnoreUsers.map(x => x.toLowerCase());
  ...
  ignoredIrcUser(user) {
    return caseInsensitiveExists(user, this.ircIgnoreUsers);
  }

  ignoredDiscordUser(user) {
    return caseInsensitiveExists(user, this.discordIgnoreUsers);
  }
