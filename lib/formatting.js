...
export function formatFromIRCToDiscord(text) {
  ...
  for (let i = 0; i <= blocks.length; i += 1) {
	...
	const forbiddenWords = ["@everyone"];

	let filteredBlockText = block.text;

	forbiddenWords.forEach(forbiddenWord => {
	  if (filteredBlockText.includes(forbiddenWord)){
		filteredBlockText = filteredBlockText.replace(forbiddenWord, '');
	  }
	});

	mdText += filteredBlockText || '';
  }
